...
extern void plat_secondary_cold_boot_bl3();
...
void __init bl31_platform_setup(void)
{
	...
	/* Move all cores to BL3 */
	uint64_t *mailbox = (uint64_t *)PLAT_ARM_TRUSTED_MAILBOX_BASE;

	/* set the core 0 mailbox to bl3 entrypoint for other cpus */
        *mailbox = (uint64_t) (uint64_t *) plat_secondary_cold_boot_bl3;

        /* make sure write are complete and send the event */
        dsb();
        sev();

	/* Invalidate the OCM now */
}
