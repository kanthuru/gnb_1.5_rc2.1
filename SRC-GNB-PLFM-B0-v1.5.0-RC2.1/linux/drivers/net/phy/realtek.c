...
{
	linkmode_mod_bit(ETHTOOL_LINK_MODE_2500baseT_Full_BIT,
			 phydev->supported, RTL_SUPPORTS_2500FULL);
...
static int rtl8221b_read_status(struct phy_device *phydev)
{
	...
	val = phy_read_mmd(phydev, MDIO_MMD_VEND2, MIIM_RTL8221B_PHY_STATUS);
	if (val < 0)
		return val;

	if (val & MIIM_RTL8221B_LINK_UP) {
		phydev->link = 1;
		speed = (val & MIIM_RTL8221B_SPEED_MASK);
		...
		if (val & MIIM_RTL8221B_FULL_DUPLEX)
