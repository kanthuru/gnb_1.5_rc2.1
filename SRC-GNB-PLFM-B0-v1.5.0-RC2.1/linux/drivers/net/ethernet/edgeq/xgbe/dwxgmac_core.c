				    ...
				    u32 chan)
{
	...
	reg = XGMAC_MTL_RXQ_DMA_MAP0 + (queue & 0xC);
	queue &= 0x3;
				    ...
				    struct xgmac_extra_stats *x)
{
	...
	if (stat & XGMAC_LINK_FAULT) {
		ret |= CORE_IRQ_LINK_FAULT;
	}
...
static int dwxgmac2_host_mtl_irq_status(struct mac_device_info *hw, u32 queue)
{
	void __iomem *ioaddr = hw->pcsr;
	int ret = 0;
	u32 status;
	...
	if (status & BIT(queue)) {
		u32 queue_status = readl(ioaddr + XGMAC_MTL_QINT_STATUS(queue));

		if (queue_status & XGMAC_RXOVFIS)
			ret |= CORE_IRQ_MTL_RX_OVERFLOW;

		writel(~0x0, ioaddr + XGMAC_MTL_QINT_STATUS(queue));
	}
...
#define STAT_OFF(field)		offsetof(struct xgmac_safety_stats, field)
...
static int dwxgmac3_safety_feat_dump(struct xgmac_safety_stats *stats,
					    ...
					    int pos)
{
	...
	/* Write first word of the data */
	val = *((u32 *)&entry->val);
	writel(val, ioaddr + XGMAC_MTL_RXP_IACC_DATA);

	/* Set pos, write OP, continuous write, and start writing */
	real_pos = pos * (sizeof(entry->val) / sizeof(u32));
	val = real_pos & XGMAC_ADDR;
	val |= (XGMAC_WRRDN | XGMAC_CRWEN | XGMAC_CRWSEL);
	val |= XGMAC_STARTBUSY;
	writel(val, ioaddr + XGMAC_MTL_RXP_IACC_CTRL_ST);

	for (i = 1; i < (sizeof(entry->val) / sizeof(u32)); i++) {
		/* Wait for done */
		ret = readl_poll_timeout(ioaddr + XGMAC_MTL_RXP_IACC_CTRL_ST,
					 val, !(val & XGMAC_STARTBUSY), 1, 10000);
		if (ret)
...
static struct xgmac_tc_entry *
dwxgmac3_rxp_get_next_entry(struct xgmac_tc_entry *entries,
...
static int dwxgmac3_rxp_config(void __iomem *ioaddr,
			       struct xgmac_tc_entry *entries,
			       unsigned int count)
{
	struct xgmac_tc_entry *entry, *frag;
	int i, ret, nve = 0;
	u32 curr_prio = 0;
	u32 old_val, val;
	...
	/* Both NVE and NPE are (num_entries - 1) */
	nve--;
...
static int dwxgmac2_get_mac_tx_timestamp(struct mac_device_info *hw, u64 *ts)
{
	...
	/* For XGMAC, read SEC reg last. For XLGMAC, read NSEC reg last */
	if (hw->xlgmac)
		value = readl(ioaddr + XGMAC_TXTIMESTAMP_SEC);
	*ts = readl(ioaddr + XGMAC_TXTIMESTAMP_NSEC) & XGMAC_TXTSSTSLO;
	if (!hw->xlgmac)
		value = readl(ioaddr + XGMAC_TXTIMESTAMP_SEC);
	*ts += value * 1000000000ULL;
...
static int dwxgmac2_fixed_pps_config(void __iomem *ioaddr, int index,
				     u32 clk_rate, u8 pps_hz)
{
	u32 reg;

	/* PPS0 rate set to 1 Hz */
	reg = readl(ioaddr + XGMAC_PPS_CONTROL);
	reg &= ~XGMAC_PPSCTRL0;
	reg |= pps_hz;
	writel(reg, ioaddr + XGMAC_PPS_CONTROL);

	return 0;
}

#define XGMAC_PPS_INTV		0x2A0904EE // 1 sec interval
#define XGMAC_PPS_PULSE_WIDTH	0x6B9C34   // 10 ms pulse width
static int dwxgmac2_gen_flex_pps(void __iomem *ioaddr, u32 step, bool stop)
{
	u32 curr_sec;
	u32 curr_nsec;
	u32 val = XGMAC_PPSEN0 | XGMAC_TRGTMOD_START_ONLY;

	val |= XGMAC_PPSCMD_STOP;
	writel(val, ioaddr + XGMAC_PPS_CONTROL);
	if (stop) {
		val = XGMAC_PPSCTRL0_1HZ;
		writel(val, ioaddr + XGMAC_PPS_CONTROL);
		return 0;
	}

	writel(XGMAC_PPS_INTV, ioaddr + XGMAC_PPSx_INTERVAL(0));
	writel(XGMAC_PPS_PULSE_WIDTH, ioaddr + XGMAC_PPSx_WIDTH(0));

	curr_sec = readl(ioaddr + XGMAC_SYSTEM_TIME_SEC);
	curr_nsec = readl(ioaddr + XGMAC_SYSTEM_TIME_NSEC);
	/* With 12 ms timer, expect nsec is in 12ms to 16ms range.
	 * If nsec is over 500ms, time has jumped, either caused by
	 * interface restart or time resync. In this case, delay one
	 * extra second for next PPS.
	 */
	if (curr_nsec > 500000000)
		step += 1;
	writel(curr_sec + step, ioaddr + XGMAC_PPSx_TARGET_TIME_SEC(0));
	writel(0, ioaddr + XGMAC_PPSx_TARGET_TIME_NSEC(0));

	val &= ~XGMAC_PPSCTRL0;
	val |= XGMAC_PPSCMD_START;
	writel(val, ioaddr + XGMAC_PPS_CONTROL);
				     ...
				     u32 match, bool dma_en, u16 dma_ch)
{
	...
	if (dma_en) {
		value &= ~XGMAC_DMCHN;
		value |= dma_ch << XGMAC_DMCHN_SHIFT;
		value |= XGMAC_DMCHEN;
	}
	else {
		value &= ~XGMAC_DMCHEN;
...
static int dwxgmac3_est_configure(void __iomem *ioaddr, struct xgmac_est *cfg,
...
static void dwxgmac2_rx_queue_routing(struct mac_device_info *hw,
				      u8 pkt_route, u32 queue, bool enable)
{
	void __iomem *ioaddr = hw->pcsr;
	u32 value;

	static const struct xgmac_rx_routing route_possibilities[] = {
		{ 0, 0 },
		{ XGMAC_PTPQ_MASK, XGMAC_PTPQ_SHIFT },
		{ 0, 0 },
		{ 0, 0 },
		{ XGMAC_MCBCQ_MASK, XGMAC_MCBCQ_SHIFT },
	};

	/* These routings are not supported */
	if (pkt_route == PACKET_AVCPQ ||
	    pkt_route == PACKET_DCBCPQ ||
	    pkt_route == PACKET_UPQ)
		return;

	value = readl(ioaddr + XGMAC_RXQ_CTRL1);

	/* routing configuration */
	value &= ~route_possibilities[pkt_route-1].reg_mask;
	if (enable)
		value |= (queue << route_possibilities[pkt_route-1].reg_shift) &
			 route_possibilities[pkt_route-1].reg_mask;

	/* some queues require to enable */
	if (pkt_route == PACKET_MCBCQ) {
		if (enable)
			value |= XGMAC_MCBCQEN;
		else
			value &= ~XGMAC_MCBCQEN;
	...
	/* tagged PTP packets are routed to PTPQ too */
	if (pkt_route == PACKET_PTPQ) {
		value &= ~XGMAC_TPQC_MASK;
		value |= XGMAC_TP_OVER_PTPQ << XGMAC_TPQC_SHIFT;
	}

	writel(value, ioaddr + XGMAC_RXQ_CTRL1);
}

static void dwxgmac2_set_jumbo_enable(void __iomem *ioaddr, bool enable, bool wd_disable)
{
	u32 value = readl(ioaddr + XGMAC_RX_CONFIG);

	if (enable)
		value |= XGMAC_CONFIG_JE;
	else
		value &= ~XGMAC_CONFIG_JE;
	if (wd_disable)
		value |= XGMAC_CONFIG_WD;
	else
		value &= ~XGMAC_CONFIG_WD;
...
static void dwxgmac2_mtl_rx_mode(void __iomem *ioaddr, int mode,
				 u32 queue, int fifosz, u8 qmode)
{
	u32 value = readl(ioaddr + XGMAC_MTL_RXQ_OPMODE(queue));
	...
	if (fifosz >= 4096 && qmode != MTL_QUEUE_AVB) {
		u32 flow = readl(ioaddr + XGMAC_MTL_RXQ_FLOW_CONTROL(queue));
		...
		writel(flow, ioaddr + XGMAC_MTL_RXQ_FLOW_CONTROL(queue));
	}

	writel(value, ioaddr + XGMAC_MTL_RXQ_OPMODE(queue));

	/* Enable MTL RX overflow */
	/* To reduce overflow intr impact, disable overflow intr */
	//value = readl(ioaddr + XGMAC_MTL_QINTEN(queue));
	//writel(value | XGMAC_RXOIE, ioaddr + XGMAC_MTL_QINTEN(queue));
}

static void dwxgmac2_mtl_tx_mode(void __iomem *ioaddr, int mode,
				 u32 queue, int fifosz, u8 qmode)
{
	u32 value = readl(ioaddr + XGMAC_MTL_TXQ_OPMODE(queue));
	...
	value |= (queue << XGMAC_Q2TCMAP_SHIFT) & XGMAC_Q2TCMAP;
...
static void dwxgmac2_qmode(void __iomem *ioaddr, u32 queue, u8 qmode)
{
	u32 value = readl(ioaddr + XGMAC_MTL_TXQ_OPMODE(queue));
...
static void dwxgmac2_set_umac_dma_chan(struct mac_device_info *hw,
				       unsigned int reg_n, u8 dma_chan)
{
	void __iomem *ioaddr = hw->pcsr;
	u32 low, high;

	low = readl(ioaddr + XGMAC_ADDRx_LOW(reg_n));

	high = readl(ioaddr + XGMAC_ADDRx_HIGH(reg_n));
	high = (high & ~XGMAC_DCS) | ((dma_chan << XGMAC_DCS_SHIFT) & XGMAC_DCS);
	writel(high, ioaddr + XGMAC_ADDRx_HIGH(reg_n));

	writel(low, ioaddr + XGMAC_ADDRx_LOW(reg_n));
}

const struct xgmac_ops dwxgmac210_ops = {
	...
	.fixed_pps_config = dwxgmac2_fixed_pps_config,
	.gen_flex_pps = dwxgmac2_gen_flex_pps,
	...
	.set_jumbo_enable = dwxgmac2_set_jumbo_enable,
	.mtl_rx_mode = dwxgmac2_mtl_rx_mode,
	.mtl_tx_mode = dwxgmac2_mtl_tx_mode,
	.qmode = dwxgmac2_qmode,
	.set_umac_dma_chan = dwxgmac2_set_umac_dma_chan,
};
...
static void dwxlgmac2_rx_queue_disable(struct mac_device_info *hw, u32 queue)
{
	void __iomem *ioaddr = hw->pcsr;
	u32 value;
...
static void dwxlgmac2_rx_queue_prio(struct mac_device_info *hw, u32 prio,
				    u32 queue)
{
	void __iomem *ioaddr = hw->pcsr;
	u32 value, reg;

	/* support upto 8 priority queues - same as XGMAC */
	if (queue >= 8)
		return;

	reg = (queue < 4) ? XLGMAC_RXQ_PRI_CTRL0 : XLGMAC_RXQ_PRI_CTRL1;
	queue &= 0x3;

	value = readl(ioaddr + reg);
	value &= ~XLGMAC_PSRQ(queue);
	value |= (prio << XLGMAC_PSRQ_SHIFT(queue)) & XLGMAC_PSRQ(queue);

	writel(value, ioaddr + reg);
}

static void dwxlgmac2_rx_queue_routing(struct mac_device_info *hw,
				       u8 pkt_route, u32 queue, bool enable)
{
	void __iomem *ioaddr = hw->pcsr;
	u32 value;
	...
	static const struct xgmac_rx_routing route_possibilities[] = {
		{ 0, 0 },
		{ XLGMAC_PTPQ_MASK, XLGMAC_PTPQ_SHIFT },
		{ 0, 0 },
		{ 0, 0 },
		{ XLGMAC_MCBCQ_MASK, XLGMAC_MCBCQ_SHIFT },
	};
	...
	if (pkt_route == PACKET_MCBCQ)
		value = readl(ioaddr + XLGMAC_RXQ_CTRL0);
	else
		value = readl(ioaddr + XLGMAC_RXQ_CTRL1);
	...
	if (pkt_route == PACKET_MCBCQ) {
		if (enable)
			value |= XLGMAC_MCBCQEN;
		else
			value &= ~XLGMAC_MCBCQEN;
	}

	if (pkt_route == PACKET_MCBCQ)
		writel(value, ioaddr + XLGMAC_RXQ_CTRL0);
	else
		writel(value, ioaddr + XLGMAC_RXQ_CTRL1);
}

const struct xgmac_ops dwxlgmac2_ops = {
	.core_init = dwxgmac2_core_init,
	.set_mac = dwxgmac2_set_mac,
	.rx_ipc = dwxgmac2_rx_ipc,
	.rx_queue_enable = dwxlgmac2_rx_queue_enable,
	.rx_queue_disable = dwxlgmac2_rx_queue_disable,
	.rx_queue_prio = dwxlgmac2_rx_queue_prio,
	.tx_queue_prio = dwxgmac2_tx_queue_prio,
	.rx_queue_routing = dwxlgmac2_rx_queue_routing,
	.prog_mtl_rx_algorithms = dwxgmac2_prog_mtl_rx_algorithms,
	.prog_mtl_tx_algorithms = dwxgmac2_prog_mtl_tx_algorithms,
	.set_mtl_tx_queue_weight = dwxgmac2_set_mtl_tx_queue_weight,
	.map_mtl_to_dma = dwxgmac2_map_mtl_to_dma,
	...
	.debug = NULL,
	.set_filter = dwxgmac2_set_filter,
	.safety_feat_config = NULL,
	.safety_feat_irq_status = NULL,
	.safety_feat_dump = NULL,
	.set_mac_loopback = dwxgmac2_set_mac_loopback,
	.rss_configure = dwxgmac2_rss_configure,
	.update_vlan_hash = dwxgmac2_update_vlan_hash,
	.rxp_config = NULL,
...
int dwxgmac2_setup(struct xgmac_priv *priv)
{
	...
	dev_info(priv->device, "XGMAC setup\n");
...
int dwxlgmac2_setup(struct xgmac_priv *priv)
{
	...
	dev_info(priv->device, "XLGMAC setup\n");
	...
	mac->link.speed_mask = XLGMAC_CONFIG_SS_MASK;
