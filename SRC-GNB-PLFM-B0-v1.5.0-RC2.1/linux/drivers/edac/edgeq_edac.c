...
/* ECCCTL UE/CE Interrupt enable/disable for raptor2*/
...
struct synps_edac_priv {
	...
	/* Circular buffer for error messages */
	char messages[MAX_ERROR_LOGS][SYNPS_EDAC_MSG_SIZE];
	/* Index for the next message */
	int msg_index;
	/* Total messages stored (max 50) */
	int total_msgs;
...
/**
 * raptor2_get_dtype - Return the controller memory width.
 ...
 */
static enum dev_type raptor2_get_dtype(const void __iomem *base)
{
	enum dev_type dt;
	u32 width;
...
/**
 * raptor2_get_error_info - Get the current ECC error info.
 * @priv:	DDR memory controller private instance data.
 *
 * Return: one if there is no error otherwise returns zero.
 */
static int raptor2_get_error_info(struct synps_edac_priv *priv)
{
...
static void handle_error(struct mem_ctl_info *mci, struct synps_ecc_status *p)
{
	...
	int index = (priv->erase_log) ? (priv->msg_index = 0) : (priv->msg_index % MAX_ERROR_LOGS);
	...
	if (p->ce_cnt) {
		...
		if (!priv->p_data->quirks) {
			...
			snprintf(priv->message, SYNPS_EDAC_MSG_SIZE,
				 "DDR ECC error type:%s Row %d Bank %d BankGroup Number %d Block Number %d Bit Position: %d Data_low: 0x%08x Data_high: 0x%08x Syndrome: 0x%08x",
	...
	if (p->ue_cnt) {
		...
		edac_mc_handle_error(HW_EVENT_ERR_UNCORRECTED, mci,
				     ...
				     priv->message, "");
		pr_err("%s", priv->message);
	}

	if (priv->ce_cnt || priv->ue_cnt) {
		/* Start logging a Correctable/UnCorrectable error */
		if (priv->logging_enable) {
			priv->stat.ce_cnt =  + priv->ce_cnt;
			priv->stat.ue_cnt =  + priv->ue_cnt;
			/* Format the message and store it in the circular buffer */
			snprintf(priv->messages[index], SYNPS_EDAC_MSG_SIZE, priv->message);
			/* Update indices */
			priv->msg_index++;

			if (priv->total_msgs < MAX_ERROR_LOGS)
				priv->total_msgs++;
		}
	}
...
static inline void enable_intr_raptor2(struct synps_edac_priv *priv)
{
	int regval;
...
static inline void disable_intr_raptor2(struct synps_edac_priv *priv)
{
	int regval;
...
static void enable_intr(struct synps_edac_priv *priv)
{
	...
	if (priv->p_data->quirks & DDR_ECC_RAPTOR2)
		return enable_intr_raptor2(priv);
}
...
static void disable_intr(struct synps_edac_priv *priv)
{
	...
	if (priv->p_data->quirks & DDR_ECC_RAPTOR2)
		return disable_intr_raptor2(priv);
}
...
static irqreturn_t intr_handler(int irq, void *dev_id)
{
	...
	/*
	 * Quirk: need to re-enable the UE/CE interrupts
	 */
	enable_intr(priv);
	return IRQ_HANDLED;
}
...
/**
 * raptor2_get_ecc_state - Return the controller ECC enable/disable status.
 * @base:	DDR memory controller base address.
 *
 ...
 */
static bool raptor2_get_ecc_state(void __iomem *base)
{
	enum dev_type dt;
	u32 ecctype;

	dt = raptor2_get_dtype(base);
	if (dt == DEV_UNKNOWN)
		return false;
...
/**
 * rapto2_get_mtype - Returns controller memory type.
 ...
 */
static enum mem_type raptor2_get_mtype(const void __iomem *base)
{
	enum mem_type mt;
	u32 memtype;
		     ...
		     struct platform_device *pdev)
{
	...
	for (i = 0; i < 2; i++) {
		...
		edac_printk(KERN_INFO, EDAC_MC, "irq request %d complete\n", irq);
	}
	enable_intr(priv);
...
static const struct synps_platform_data raptor2_edac_def = {
	.get_error_info	= raptor2_get_error_info,
	.get_mtype	= raptor2_get_mtype,
	.get_dtype	= raptor2_get_dtype,
	.get_ecc_state	= raptor2_get_ecc_state,
	.quirks         = (DDR_ECC_INTR_SUPPORT | DDR_ECC_RAPTOR2
#ifdef CONFIG_EDAC_DEBUG
...
static ssize_t error_log_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	index = (priv->msg_index - priv->total_msgs + MAX_ERROR_LOGS) % MAX_ERROR_LOGS;
	for (i = 0; i < priv->total_msgs; i++) {
		count += snprintf(buf + count, SYNPS_EDAC_MSG_SIZE, "%s\n", priv->messages[index]);
		index = (index + 1) % MAX_ERROR_LOGS;
	}

	return count;
}

static ssize_t error_stat_log_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mem_ctl_info *mci = to_mci(dev);
	struct synps_edac_priv *priv = mci->pvt_info;

	return sprintf(buf, "Total error count CE %d UE %d priv->msg_index %d\n\r",
	 priv->ce_cnt, priv->ue_cnt, priv->msg_index);
}

static ssize_t logging_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mem_ctl_info *mci = to_mci(dev);
	struct synps_edac_priv *priv = mci->pvt_info;
...
static ssize_t logging_enable_store(struct device *dev,
					...
					const char *buf, size_t count)
{
	...
	if (kstrtoint(buf, 10, &new_val))
		return -EINVAL;

	if ((new_val == 0) || (new_val == 1))
		priv->logging_enable = new_val;

	return count;
}

static ssize_t erase_log_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct mem_ctl_info *mci = to_mci(dev);
	struct synps_edac_priv *priv = mci->pvt_info;
	...
	if ((new_val == 0) || (new_val == 1))
		priv->erase_log = new_val;

	if (priv->erase_log)
		memset(priv->messages, 0, sizeof(priv->messages));
...
static DEVICE_ATTR_RO(error_stat_log);
static DEVICE_ATTR_WO(erase_log);
static DEVICE_ATTR_RW(logging_enable);
...
static int edac_create_sysfs_attributes(struct mem_ctl_info *mci)
{
	...
	rc = device_create_file(&mci->dev, &dev_attr_error_log);
	if (rc < 0)
		return rc;

	rc = device_create_file(&mci->dev, &dev_attr_error_stat_log);
	if (rc < 0)
		return rc;

	rc = device_create_file(&mci->dev, &dev_attr_erase_log);
	if (rc < 0)
		return rc;

	rc = device_create_file(&mci->dev, &dev_attr_logging_enable);
	if (rc < 0)
		return rc;
...
{
	device_remove_file(&mci->dev, &dev_attr_error_log);
	device_remove_file(&mci->dev, &dev_attr_error_stat_log);
	device_remove_file(&mci->dev, &dev_attr_erase_log);
	device_remove_file(&mci->dev, &dev_attr_logging_enable);
...
MODULE_AUTHOR("EdgeQ");
MODULE_DESCRIPTION("DDR ECC driver");
