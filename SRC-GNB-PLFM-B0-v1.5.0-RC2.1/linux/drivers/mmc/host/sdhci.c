...
#if IS_ENABLED(CONFIG_MMC_SDHCI_OF_RAPTOR2)
#define BSS_RAM_START 0x80000000
#define E1_RAM_START 0x400000000ULL
#define E1_TO_BSS(addr) ((uint32_t)((addr - E1_RAM_START) + BSS_RAM_START))
/* On Raptor2 B0 Titan target DDR address goes to NOC should be 4GB less */
#define DDR_ADDR_OFFSET		0x100000000ULL
#define DDR_ADDR_XLAT(addr)	((uint64_t)(addr - DDR_ADDR_OFFSET))
#endif
				  ...
				  struct mmc_data *data, int cookie)
{
	...
	if (host->bounce_buffer) {
		...
		if (!(host->quirks2 & SDHCI_QUIRK2_RAPTOR2_NON_COHERENT)) {
			   ...
			   dma_addr_t addr, int len, unsigned int cmd)
{
	...
	if (host->quirks2 & SDHCI_QUIRK2_RAPTOR2_COHERENT)
		addr = DDR_ADDR_XLAT(addr);
...
static void sdhci_set_adma_addr(struct sdhci_host *host, dma_addr_t addr)
{
	...
	if (host->quirks2 & SDHCI_QUIRK2_RAPTOR2_NON_COHERENT) {
		u32 lower_32bits = lower_32_bits(E1_TO_BSS(addr));
		u32 upper_32bits = upper_32_bits(E1_TO_BSS(addr));

		if (mmc_get_dma_dir(host->data) == DMA_TO_DEVICE)
			__flush_dcache_area(host->bounce_buffer, host->bounce_buffer_size);

		sdhci_writel(host, lower_32bits, SDHCI_ADMA_ADDRESS);
		if (host->flags & SDHCI_USE_64_BIT_DMA)
			sdhci_writel(host, upper_32bits, SDHCI_ADMA_ADDRESS_HI);
	} else {
		   ...
		   unsigned int *actual_clock)
{
	...
	if (host->version >= SDHCI_SPEC_300) {
		...
		if (!host->clk_mul || switch_base_clk) {
			...
			real_div = div;
			div >>= 1;
			if (host->quirks2 & SDHCI_QUIRK2_RAPTOR2_A0) {
				if ((host->quirks2 & SDHCI_QUIRK2_CLOCK_DIV_ZERO_BROKEN)
						&& !div && host->max_clk <= 25000000)
					div = 1;
				else if ((host->quirks2 & SDHCI_QUIRK2_CLOCK_DIV_ZERO_BROKEN) &&
						!div && host->max_clk <= 46000000)
					div = 3;
			} else if (host->quirks2 & SDHCI_QUIRK2_RAPTOR2_B0) {
				if ((host->quirks2 & SDHCI_QUIRK2_CLOCK_DIV_ZERO_BROKEN) &&
						!div && host->max_clk <= 23000000)
					div = 3;
				else if ((host->quirks2 & SDHCI_QUIRK2_CLOCK_DIV_ZERO_BROKEN) &&
						!div && host->max_clk <= 46000000)
					div = 5;
		...
		if (data && data->host_cookie == COOKIE_MAPPED) {
			if (host->bounce_buffer) {
				...
				if (mmc_get_dma_dir(data) == DMA_FROM_DEVICE) {
					...
					if ((host->quirks2 & SDHCI_QUIRK2_RAPTOR2_B0) &&
					(host->quirks2 & SDHCI_QUIRK2_RAPTOR2_NON_COHERENT))
						__inval_dcache_area(host->bounce_buffer,
								host->bounce_buffer_size);
...
static int sdhci_set_dma_mask(struct sdhci_host *host)
{
	...
	} else if (host->quirks2 & SDHCI_QUIRK2_RAPTOR2_NON_COHERENT) {
...
static void sdhci_allocate_bounce_buffer(struct sdhci_host *host)
{
	...
	if (host->quirks2 & SDHCI_QUIRK2_RAPTOR2_NON_COHERENT) {
		host->bounce_addr = host->paddr;
		host->bounce_buffer = (char *)host->vaddr;
	} else {
		/*
