...
static irqreturn_t gnss_irq_handler(int irq, void *dev_id)
{
	struct gnss_device *gdev = (struct gnss_device *)dev_id;
	static unsigned long prev_jiffies; // for debugging purpose
	unsigned long curr_jiffies;


	if (gdev->pps_irq_dbg) {
		curr_jiffies = jiffies;
		dev_dbg(&gdev->dev, "GNSS interrupt received\n");
		if ((prev_jiffies != 0) && (curr_jiffies - prev_jiffies) < (HZ * 3 / 4)) {
			dev_dbg(&gdev->dev, "curr: %lu, prev: %lu, curr - prev: %lu\n", curr_jiffies,
					prev_jiffies, (curr_jiffies - prev_jiffies));
		}
		prev_jiffies = jiffies;
	}

	if (gdev->pps_irq_rcvd == 0) {
		gdev->pps_irq_rcvd = 1;
		wake_up_interruptible(&gdev->pps_wait);
	}
...
static long gnss_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	struct gnss_device *gdev = file->private_data;
	int ret;

	switch (cmd) {
		case GNSS_IOCTL_WAIT_PPS_IRQ:
			if (!gdev->pps_intr_enable) {
				dev_err(&gdev->dev, "GNSS IRQ was not enabled!\n");
				return -EPERM;
			}
			gdev->pps_irq_rcvd = 0;
			ret = wait_event_interruptible(gdev->pps_wait, gdev->pps_irq_rcvd);
			if (ret == -ERESTARTSYS) {
				dev_dbg(&gdev->dev, "PPS irq wait was interrupted!\n");
			}
			break;
		default:
			dev_err(&gdev->dev, "Unkown cmd %u\n", cmd);
			break;
	}
...
{
	gdev->pps_intr_enable = 0;
	free_irq(gdev->pps_irq, gdev);

	return 0;
}
EXPORT_SYMBOL(gnss_deregister_pps_irq);

int gnss_register_pps_irq(struct gnss_device *gdev, struct device dev)
{
	struct gpio_desc *gpio;
	struct pinctrl *pinctrl;
	struct pinctrl_state *pinctrl_state_pps_irq;
	int ret;

	/* Get the pinctrl handle from the device */
	pinctrl = devm_pinctrl_get(&dev);
	if (IS_ERR(pinctrl)) {
		dev_err(&gdev->dev, "Failed to get pinctrl\n");
		return PTR_ERR(pinctrl);
	}

	/* Lookup the pin state */
	pinctrl_state_pps_irq = pinctrl_lookup_state(pinctrl, "sif_input");
	if (IS_ERR(pinctrl_state_pps_irq)) {
		dev_err(&gdev->dev, "Failed to lookup default pinctrl pps state\n");
		return PTR_ERR(pinctrl_state_pps_irq);
	}

	/* Select the pin state */
	ret = pinctrl_select_state(pinctrl, pinctrl_state_pps_irq);
	if (ret) {
		dev_err(&gdev->dev, "Failed to select default pinctrl pps state\n");
		return ret;
	}

	gpio = devm_gpiod_get(&dev, "gnss", GPIOD_ASIS);
	if (IS_ERR(gpio)) {
		dev_err(&gdev->dev, "Failed to get GPIO descriptor from gnss\n");
		return PTR_ERR(gpio);
	}

	/* Init PPS wait */
	init_waitqueue_head(&gdev->pps_wait);

	/* Register IRQ */
	gdev->pps_irq = gpiod_to_irq(gpio);
	if (gdev->pps_irq <= 0) {
		dev_err(&gdev->dev, "Failed to get IRQ number\n");
		return -1;
	}

	if (request_irq(gdev->pps_irq, gnss_irq_handler,
			  IRQF_TRIGGER_RISING, "gnss", gdev) < 0) {
		dev_err(&gdev->dev, "Failed to request IRQ\n");
		return -1;
	}

	gdev->pps_intr_enable = 1;

	return 0;
}
EXPORT_SYMBOL(gnss_register_pps_irq);
