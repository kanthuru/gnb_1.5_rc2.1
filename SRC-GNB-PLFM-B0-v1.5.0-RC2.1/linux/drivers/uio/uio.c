...
static int uio_mmap_physical(struct vm_area_struct *vma)
{
	...
	if (idev->info->mem[mi].memtype == UIO_MEM_PHYS_PFETCH)
		vma->vm_page_prot = pgprot_writecombine(vma->vm_page_prot);
#ifdef CONFIG_ARM64
	if (idev->info->mem[mi].memtype == UIO_MEM_PHYS_PFETCH_NS) {
		vma->vm_page_prot.pgprot &= ~(3ULL << 8);
	}
#endif
