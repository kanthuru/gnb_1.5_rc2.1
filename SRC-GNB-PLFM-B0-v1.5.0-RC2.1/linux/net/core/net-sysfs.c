...
#ifdef CONFIG_RPS_HOUSEKEEPING_CPUS
...
static ssize_t show_rps_fixed_cpu(struct netdev_rx_queue *queue, char *buf)
{
	int len;

	rcu_read_lock();
	len = snprintf(buf, PAGE_SIZE, "%*pb\n",
		       cpumask_pr_args(&queue->rps_fixed_cpu));
	rcu_read_unlock();

	return len < PAGE_SIZE ? len : -EINVAL;
}

static ssize_t store_rps_fixed_cpu(struct netdev_rx_queue *queue,
				   const char *buf, size_t len)
{
	cpumask_var_t mask;
	int err;
	static DEFINE_MUTEX(rps_fixed_cpu_mutex);
	...
	mutex_lock(&rps_fixed_cpu_mutex);
	queue->rps_fixed_cpu = *mask;
	mutex_unlock(&rps_fixed_cpu_mutex);
...
static struct rx_queue_attribute rps_fixed_cpu_attribute __ro_after_init =
	__ATTR(rps_fixed_cpu, 0644, show_rps_fixed_cpu, store_rps_fixed_cpu);
#endif /* CONFIG_RPS */
