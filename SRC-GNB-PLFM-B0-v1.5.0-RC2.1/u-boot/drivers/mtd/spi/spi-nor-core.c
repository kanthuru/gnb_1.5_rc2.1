...
static int write_enable(struct spi_nor *nor)
{
	...
	if (ret < 0) {
		debug("SF: Failed to set WEL via WREN\n");
		return ret;
	}
...
/**
 * Reset a SPI NOR flash
 *
 * @param nor flash context pointer
 * @return 0 if success, else negative value
 */
static int spi_nor_reset(struct spi_nor *nor)
{
	int ret;

	/* Reset Enable command must be followed by Reset command */
	ret = nor->write_reg(nor, SPINOR_OP_RSTEN, NULL, 0);
	if (ret < 0)
		goto failure;

	ret = nor->write_reg(nor, SPINOR_OP_RST, NULL, 0);
	if (ret < 0)
		goto failure;
...
failure:
	debug("SF: failed to reset flash\n");
	return ret;
}

/**
 * Completely erase a SPI NOR flash
 *
 * @param nor flash context pointer
 * @return 0 if success, else negative value
 */
static int spi_nor_chip_erase(struct spi_nor *nor)
{
	int ret;

	/* If device doesn't support chip erase */
	if (nor->flags & SNOR_F_NO_OP_CHIP_ERASE) {
		printf("SF: chip erase is not available for this device\n");
		return -EPERM;
	}
	...
	if (ret < 0) {
		debug("SF: failed to erase flash chip\n");
		return ret;
	}

	/* Wait till chip erase is done => Wait till WIP bit in SR is unset */
	while (!spi_nor_sr_ready(nor))
		;
...
static const struct flash_info *spi_nor_read_id(struct spi_nor *nor)
{
	...
	tmp = nor->read_reg(nor, SPINOR_OP_RDID, id, SPI_NOR_JEDEC_ID_LEN);
	if (tmp < 0) {
...
int spi_nor_scan(struct spi_nor *nor)
{
	...
	nor->reset = spi_nor_reset;
	nor->chip_erase = spi_nor_chip_erase;
