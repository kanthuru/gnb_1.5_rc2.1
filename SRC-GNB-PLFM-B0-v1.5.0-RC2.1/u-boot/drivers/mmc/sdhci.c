...
#if CONFIG_IS_ENABLED(MMC_SDHCI_RAPTOR2)
	timeout = 10000000;
#else
	timeout = 1000000;
#endif
...
int sdhci_set_clock(struct mmc *mmc, unsigned int clock)
{
	...
	if (SDHCI_GET_VERSION(host) >= SDHCI_SPEC_300) {
		...
		if (host->clk_mul) {
			...
			div >>= 1;
			if (((host->quirks & SDHCI_QUIRK_CLOCK_DIV_ZERO_BROKEN) &&
						CONFIG_IS_ENABLED(MMC_SDHCI_RAPTOR2))
                                && !div && host->max_clk == 23000000)
			{
				div = 1;
			}
			else if (((host->quirks & SDHCI_QUIRK_CLOCK_DIV_ZERO_BROKEN) &&
						CONFIG_IS_ENABLED(MMC_SDHCI_RAPTOR2))
	...
	if(clock == host->max_clk)
		clk |= SDHCI_CLOCK_PLL_EN | SDHCI_CLOCK_CARD_EN |
			SDHCI_CLOCK_INT_STABLE | SDHCI_CLOCK_INT_EN;
	else
		clk = SDHCI_CLOCK_PLL_EN | SDHCI_CLOCK_CARD_EN |
			SDHCI_CLOCK_INT_STABLE | SDHCI_CLOCK_INT_EN;
