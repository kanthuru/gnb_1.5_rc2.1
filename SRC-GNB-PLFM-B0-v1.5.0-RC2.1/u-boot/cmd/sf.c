...
static int do_spi_flash_reset(void)
{
	int ret;

	ret = flash->reset(flash);
	if (ret) {
		printf("SF: Failed to reset flash device\n");
		return -1;
	}

	printf("SF: Successfully reset flash\n");
	return 0;
}

static int do_spi_flash_chip_erase(void)
{
	int ret;

	ret = flash->chip_erase(flash);
	if (ret) {
		printf("SF: Failed to erase flash chip\n");
		return -1;
	}

	printf("SF: Successfully erased flash\n");
	return 0;
}
			...
			char *const argv[])
{
	...
	if (strcmp(cmd, "read") == 0 || strcmp(cmd, "write") == 0 ||
		...
		ret = do_spi_flash_reset();
	else if (strcmp(cmd, "chip_erase") == 0)
		ret = do_spi_flash_chip_erase();
#ifdef CONFIG_CMD_SF_TEST
...
U_BOOT_CMD(
	...
	"					  at address 'sector'\n"
	"sf reset				- reset flash device\n"
	"sf chip_erase				- erase the entire SPI flash chip\n"
	SF_TEST_HELP
);
