...
int do_bootzf(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	unsigned long addr; /* Address of the ELF image */
	unsigned long mpidr; /* MPIDR of booting cpu */
	unsigned long entry; /* Entry point for image */
	char *sload = NULL;
	const char *ep = env_get("autostart");
	int rcode = 0;
	...
	/* Check for mpidr of booting cpu. */
	if (argc >= 1 && strict_strtoul(argv[0], 16, &mpidr) != -EINVAL) {
		/* Consume address */
		argc--; argv++;
	} else {
		printf("Error !! Please provide MPIDR of booting CPU\n");
		return 1;
	}
	...
	if (!valid_elf_image(addr)) {
		...
		entry = (unsigned long)phdr->p_paddr;
	...
	printf("## Starting application at 0x%08lx on cpu 0x%08lx...\n", addr, mpidr);

	arm_smccc_smc(0xc4000003, mpidr, entry, 0, 0, 0, 0, 0, &res);
	while(1);
...
U_BOOT_CMD(
	bootzf, CONFIG_SYS_MAXARGS, 0, do_bootzf,
	"Boot Zephyr from an ELF / binary image",
	"[-p|-s] [address] [mpidr]\n"
	"\t- load Zephyr ELF image at [address] via program headers (-p)\n"
	"\t  or via section headers (-s) and use [mpidr] as primary boot cpu"
);
