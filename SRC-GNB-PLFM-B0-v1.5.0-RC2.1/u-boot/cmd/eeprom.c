...
#define	EEPROM_ARRAY_OFFSET		0x3000 /* Upper 1/4 3000h-3FFFh */
			   ...
			   uchar *buffer, unsigned len, bool read)
{
	...
	if (i2c_set_chip_flags(dev, DM_I2C_CHIP_WR_ADDRESS))
		return CMD_RET_FAILURE;
				  ...
				  char *value, ulong addr, ulong off, ulong cnt)
{
	...
	const char *const fmt =
		"\nEEPROM @0x%lX %s: addr 0x%lx  off 0x%04lx  count %ld ... ";
#ifdef CONFIG_CMD_EEPROM_LAYOUT
...
#ifdef CONFIG_CMD_EEPROM_LAYOUT
	rcode = eeprom_read(i2c_addr, EEPROM_ARRAY_OFFSET, eeprom_buf, CONFIG_SYS_EEPROM_SIZE);
	if (rcode < 0)
		return rcode;
	...
	rcode = eeprom_write(i2c_addr, EEPROM_ARRAY_OFFSET, layout.data, CONFIG_SYS_EEPROM_SIZE);
#endif
...
int do_eeprom(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (action == EEPROM_PRINT || action == EEPROM_UPDATE) {
		if (argv[index] && (strlen(argv[index]) >= 2) && (!strncmp(argv[index], "-l", 2))) {
			NEXT_PARAM(argc, index);
